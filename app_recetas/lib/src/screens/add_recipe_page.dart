...
import 'package:app_recetas/src/components/ingredient_widget.dart';
...
class _AddRecipePageState extends State<AddRecipePage> {
  ...
  void _onIngredientDelete(int index) {
    questionDialog(context, "¿Esta seguro de eliminar el ingrediente?", () {
      setState(() {
  ...
  void _onStepDelete(int index) {
    questionDialog(context, "¿Esta seguro de eliminar el paso?", () {
      setState(() {
  ...
  void _save(BuildContext context) async {
    if (formKey.currentState.validate()) {
      ...
      if (imageFile == null) {
        _showSnackBar("La imagen está vacía");
        return;
      }
      if (ingredientsList.length == 0) {
